...
SCENE_MODULES: list[dict[str, Any]] = [
    ...
    _module(
        "fraud_script_reconstruction",
        "诈骗话术过程还原",
        ["电信网络诈骗与预警劝阻", "网络与平台交易纠纷"],
        ["诈骗", "话术", "聊天记录", "通话", "冒充", "诱导", "转账", "下载", "屏幕共享"],
        [("话术链路复盘", "根据聊天、通话和转账节点还原诈骗人员如何取得信任、施压和诱导操作。"), ("风险节点标注", "标出验证码、屏幕共享、贷款借款、继续转账等关键风险节点。")],
        ["被害人", "报警人", "相关人员"],
        "investigation",
        "高",
        "案件基础信息已掌握，训练重点转为对诈骗过程的复盘、归纳和风险节点识别。",
        "case_review",
    ),
    _module(
        "evidence_chain_review",
        "案件证据链梳理",
        ["电信网络诈骗与预警劝阻", "盗窃等侵财警情", "打架斗殴与故意伤害", "高风险侵害和人身财产案件"],
        ["证据", "监控", "聊天记录", "转账", "伤情", "痕迹", "证人", "物证"],
        [("证据清单梳理", "按人证、物证、电子数据、视听资料等类型整理现有证据和缺口。"), ("事实证据对应", "把关键事实与可支撑证据逐项对应，标出仍需补证的环节。")],
        ["被害人", "证人", "相关人员"],
        "investigation",
        "中等",
        "训练重点不是继续追问群众，而是把已有材料整理成可复核的证据链。",
        "case_review",
    ),
    _module(
        "station_statement_record",
        "回所询问笔录制作",
        ["纠纷求助与现场稳控", "盗窃等侵财警情", "打架斗殴与故意伤害", "高风险侵害和人身财产案件", "交通警情与酒驾醉驾"],
        ["笔录", "回所", "询问", "陈述", "被害人", "证人", "嫌疑人", "驾驶人"],
        [("笔录要素整理", "围绕身份、时间地点、经过、损失伤情、证据来源和诉求整理询问提纲。"), ("矛盾缺口补问", "对现场处置中遗留的矛盾点和缺失事实进行补充询问。")],
        ["被害人", "证人", "嫌疑人", "驾驶人", "相关人员"],
        "investigation",
        "中等",
        "现场处置已结束，训练重点转入回所后的规范询问和笔录要素补齐。",
        "record",
    ),
    _module(
        "interagency_transfer",
        "案件协同流转处置",
        ["电信网络诈骗与预警劝阻", "交通警情与酒驾醉驾", "走失、自伤和救助类警情", "校园与未成年人警情"],
        ["反诈中心", "止付", "冻结", "医院", "学校", "家属", "交警", "刑侦", "移交", "联动"],
        [("协同单位判断", "根据案件类型和风险判断需要联动的单位、部门和流转节点。"), ("移交材料准备", "整理需移交的事实摘要、证据材料、处置经过和后续风险提示。")],
        ["报警人", "被害人", "家属", "相关人员"],
        "investigation",
        "中等",
        "训练重点是处置闭环中的协同判断和材料移交，不局限于现场对话。",
        "coordination",
    ),
    _module(
        "post_case_feedback",
        "处置结果回访反馈",
        ["纠纷求助与现场稳控", "电信网络诈骗与预警劝阻", "走失、自伤和救助类警情", "校园与未成年人警情"],
        ["回访", "反馈", "告知", "防范", "调解", "家属", "风险复核", "宣教"],
        [("处置结果告知", "向当事人说明已采取措施、后续流程、注意事项和权利义务。"), ("风险复核回访", "复核是否仍有冲突、诈骗、轻生或未成年人保护等后续风险。")],
        ["报警人", "被害人", "家属", "相关人员"],
        "investigation",
        "低",
        "案件进入收尾阶段，训练重点是规范告知、风险复核和经验沉淀。",
        "follow_up",
    ),
]
    ...
    compact = [
        {
            ...
            "training_form": item.get("training_form", "dialogue"),
        }
        for item in modules
    ]
    return (
        "下面是根据案件类型和案情关键词选出的候选训练模块。它们只用于丰富 required_scene_slots 的训练内容，"
        "不决定场景数量，也不能逐条照搬或取代主场景、接警/研判和后续调查的组合结构。"
        "你必须按槽位职责根据案件事实自动组合、改名、删减或合并。"
        "学员固定为民警；除接警外，训练入口必须位于案件主要行为发生后，且不得改变既定案件结果。"
        "若某模块缺少事实支撑，必须舍弃。\n"
        f"{compact}"
    )
