...
.role-card__body { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px 16px; padding: 0 16px 18px; }
.role-card__body section {
  min-width: 0;
  padding: 14px;
...
.role-card h3 { margin: 0 0 8px; font-size: 15px; font-weight: 700; letter-spacing: 0; }
...
.role-card__editor { grid-column: 1 / -1; margin-top: 6px; border: 1px solid #e5e5ea; border-radius: 8px; background: #fff; padding: 0 14px; }
.role-card__editor summary { min-height: 44px; color: #007aff; cursor: pointer; font-weight: 600; }
.role-card__remove { grid-column: 1 / -1; min-height: 44px; margin-top: 4px; padding: 0; border: 0; background: transparent; color: #ff3b30; cursor: pointer; }
@media (max-width: 720px) {
  .role-card__toggle { grid-template-columns: auto minmax(0, 1fr) 20px; }
  .role-card__count { display: none; }
  .role-card__body { grid-template-columns: minmax(0, 1fr); }
}
</style>
