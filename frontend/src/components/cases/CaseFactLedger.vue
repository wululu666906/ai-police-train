...
<template>
  <section class="fact-ledger">
    <header class="fact-ledger__head">
      <button type="button" class="fact-ledger__toggle" :aria-expanded="expanded" @click="expanded = !expanded">
        <div>
          <div class="fact-ledger__eyebrow">事实抽取</div>
          <h3>案件事实账本</h3>
        </div>
        <span>{{ facts.length }} 条事实 · {{ expanded ? '收起' : '展开' }}</span>
      </button>
    </header>
    ...
    <div v-if="expanded">
      ...
      <div v-else class="fact-ledger__empty">当前没有可用事实，请复核剧情解析后再发布。</div>
    </div>
  </section>
</template>
...
.fact-ledger__head { margin-bottom: 0; }
.fact-ledger__toggle { display: flex; width: 100%; align-items: center; justify-content: space-between; gap: 16px; padding: 0; border: 0; background: transparent; text-align: left; cursor: pointer; }
.fact-ledger__head h3 { margin: 3px 0 0; color: #0f172a; font-size: 17px; letter-spacing: 0; }
.fact-ledger__toggle > span { color: #475569; font-size: 13px; white-space: nowrap; }
.fact-ledger__eyebrow { color: #2563eb; font-size: 12px; font-weight: 700; }
.fact-ledger__list { display: grid; gap: 0; margin-top: 12px; border-top: 1px solid #e2e8f0; }
...
.fact-ledger__body p { margin: 8px 0 0; color: #0f172a; line-height: 1.65; overflow-wrap: anywhere; }
.fact-ledger__meta { display: flex; flex-wrap: wrap; gap: 8px; color: #64748b; font-size: 12px; }
.fact-ledger__meta span { padding: 2px 8px; border-radius: 999px; background: #f1f5f9; }
.fact-ledger__known { margin-top: 8px; color: #475569; font-size: 12px; }
.fact-ledger__empty { margin-top: 12px; color: #64748b; font-size: 14px; }
</style>
