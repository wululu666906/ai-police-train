...
export function useFaceIdentityVerify() {
  ...
  async function postFrame(endpoint: 'verify' | 'heartbeat') {
    ...
    try {
      const result = await request.post(url, payload, { _skipErrorToast: true } as any) as unknown as FaceVerifyResult
  ...
  function startHeartbeat(retryMs = FACE_HEARTBEAT_RETRY_MS) {
    stopHeartbeat()
