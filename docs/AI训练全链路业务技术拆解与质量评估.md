...
| D05 多角色仿真与状态迁移 | Orchestrator、RoleSimulationSkill、TinyTroupe、PoliceTrainingWorld、世界快照、四维状态机 | 多角色世界推演、公开事件广播、角色内在记忆、自动发言/沉默、事实披露、情绪/配合/风险/清晰度迁移 | 角色仿真Prompt受人物记忆、事实ID、场景目标和回答边界约束；台词只取TinyTroupe `TALK`，LLM不得二次改写；发言编排由TinyWorld负责 | 架构隔离和角色边界较强；每轮多角色行动会放大模型调用成本、时延和失败概率 |
